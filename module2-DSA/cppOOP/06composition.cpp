#include <iostream>
#include <cmath>
using namespace std;

class Point {
    private:
        int x;
        int y;

    public: 
        int getX() const {return x;} // need to use "const" keyword on methods that you want to call on "const references"
        void setX(int newX) {x = newX;}

        int getY() const {return y;}
        void setY(int newY) {y = newY;} // we almost never use "const" on set methods

        void print() const {
            cout << "(" << x << "," << y << ")" << endl;
        }
        double distanceTo(const Point& other) const {
            int xDiff = x - other.x;
            int yDiff = y - other.y;
            return sqrt(xDiff * xDiff + yDiff * yDiff);
        }
    
        Point() : x(0), y(0) {}
        Point(int x, int y) : x(x), y(y) {}
};

class City {
    Point location; // Nested class
    string name;
    int population;

    public: 
        City() : location(Point()), name("unkown"), population(0) {}
        City(const Point& location, const string& name, int population) 
            : location(location), name(name), population(population) {}
        
        const Point& getLocation() {return location;} // returning the memAddress but with read-only restriction!
        void setLocation(Point& location) { this->location = location; }

        string getName() { return name; }
        void setName(string name) { this->name = name; }

        int getPopulation() { return population; }
        void setPopulation(int population) { this->population = population; }
};

int main() { 

    City city1 = City(Point(5, 10), "Betul", 300);
    const Point& location = city1.getLocation(); // need to match read-only memAdd in read-only reference 
    // location.setX(50); // ERROR : setting/modifying a const data
    
    cout << location.getX() << endl;
    cout << city1.getLocation().getX() << endl;
    
    City city2 = City(Point(100, 300), "Indore", 900);
    Point p = Point(600, 900);
    city2.setLocation(p);
    cout << city2.getLocation().getX() << endl;
    city2.setPopulation(9000);
    cout << city2.getPopulation() << endl;

    return 0;
}